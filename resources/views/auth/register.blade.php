...
@section('title', 'Register | PS Billing POS')

@section('content')
    <h2 class="font-display text-3xl uppercase tracking-[0.12em] text-white">Buat Akun</h2>
    <p class="mt-2 text-slate-400">Tambahkan operator baru ke aplikasi billing.</p>

    <form method="POST" action="{{ route('register') }}" class="mt-8 space-y-5">
        @csrf
        <div>
            <label class="mb-2 block text-sm font-semibold text-slate-300">Nama</label>
            <input name="name" type="text" value="{{ old('name') }}" required class="block w-full rounded-2xl border border-white/10 bg-slate-950/70 px-4 py-3 text-white focus:border-cyan-400 focus:ring-cyan-400">
